...
class LongSpeicher30 extends AbstractLongSpeicher {
	...
	EDK.data = n;
	Knoten k = ADK;
	//while, solange bis der Vorgänger gefunden wird
	while (k.next.data != n)
	    k = k.next;
	...
	ADK.next = new Knoten(ADK.next, n); //neuer Knoten wird immer direkt nach ADK eingefügt
	return true;
    }
	...
	Knoten vorgaengerKnoten = vorgaenger(n);
	if (vorgaengerKnoten.next == EDK) return false; //Wenn bei dem Vorgängerknoten der nächste Knoten EDK ist, dann wird false zurückgegeben

	vorgaengerKnoten.next = vorgaengerKnoten.next.next; //Vorgänger von zu löschendem Knoten zeigt jetzt auf den Nachfolger des zu löschenden Knoten 
							    //(somit wird er übersprungen und ist "gelöscht")
	return true;
    }
	...
	printf("lsc.istDrin(70): %s%n", lsc.istDrin(70));
	printf("lsc.istDrin(80): %s%n", lsc.istDrin(80));
	printf("lsc.istDrin(50): %s%n", lsc.istDrin(50));
	printf("lsc.istDrin(40): %s%n", lsc.istDrin(40));
	printf("lsc.istDrin(0): %s%n", lsc.istDrin(0));
	printf("lsc.istDrin(80): %s%n", lsc.istDrin(80));
	printf("lsc.istDrin(99): %s%n", lsc.istDrin(99));
	printf("lsc.istDrin(5000): %s%n", lsc.istDrin(5_000));
	...
	printf("lsc.loesche(220): %s%n", lsc.loesche(220));
